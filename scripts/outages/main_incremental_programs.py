...
mapping = {
    'message_id': 'publication_id', 'version': 'publication_version', 
    'date_publication_utc': 'publication_dt (UTC)', 'nom_unite': 'unit_name', 
...
if df_unit.empty:
    print(f"⚠️ Unité {UNIT_TO_PLOT} non trouvée.")
else:
    # On trie par version pour l'aspect incrémental
    df_unit = df_unit.sort_values('publication_version')
    
    plt.figure(figsize=(12, 6))
    
    # On boucle sur chaque version du message pour tracer l'évolution
    for version in df_unit['publication_version'].unique():
        df_ver = df_unit[df_unit['publication_version'] == version]
        
        # Simulation du programme : On trace la puissance dispo
        # Pour un programme "attendu", on prend la date de pub et la puissance
        label = f"Version {version} ({df_ver['publication_dt (UTC)'].iloc[0].strftime('%d/%m %H:%M')})"
        
        # On crée un petit escalier pour simuler la disponibilité
        plt.step([df_ver['publication_dt (UTC)'].iloc[0], 
                  df_ver['publication_dt (UTC)'].iloc[0] + pd.Timedelta(days=5)], 
                 [df_ver['available capacity (MW)'].iloc[0], 
                  df_ver['available capacity (MW)'].iloc[0]], 
                 where='post', label=label)

    plt.title(f"Évolution Incrémentale de la Disponibilité - {UNIT_TO_PLOT}")
    plt.xlabel("Date")
    ...
    print(f"✅ Succès ! Le graphique 'maison' est ici : {out_file}")
    plt.show()