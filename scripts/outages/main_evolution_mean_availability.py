...
# 🚨 FILTRE DRASTIQUE POUR LA MÉMOIRE (7 JOURS) 🚨
dernier = df[COL_PUB].max()
df = df[df[COL_PUB] > (dernier - pd.Timedelta(days=7))]
print(f"Nombre de messages après filtre : {len(df)}")
...
print("--- Calcul (Mode Allégé) ---")
try:
    # On calcule les programmes
    dikt_programs_raw, _ = outages.tools.compute_all_programs(df)
    
    # On vide la mémoire des unités vides et on resample en '1h'
    dikt_programs = {}
    for u, d in dikt_programs_raw.items():
        if d is not None and not d.empty:
            # On réduit la taille AVANT la fusion
    ...
    # On retire les arguments 'data_source', 'map_code' et 'production_source' 
    # car ils semblent ne plus être supportés par cette fonction dans ta version.
    outages.plot.evolution_mean_availability(
        dh,
        folder_out=FOLDER_OUT,
        close=False
    )
    print(f"\n✅ Graphique généré avec succès dans : {FOLDER_OUT}") 
